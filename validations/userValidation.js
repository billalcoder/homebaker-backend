...
export const userValidation = z.object({
  ...
  email: z.string().trim().email("Invalid email format").min(5, "Email must be at least 5 characters long"),
  ...
  password: z.string().trim().min(4, "Password must be at least 4 characters long")
    .refine((val) => /[A-Z]/.test(val) && /[a-z]/.test(val) && /[0-9]/.test(val) && /[@$!%*?&#]/.test(val), {
      message: "Password must include uppercase, lowercase, number, and special character",
    }),

  // New Address Validation Object
  address: z.object({
